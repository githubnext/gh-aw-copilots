...
- **Default restrictions**: If `output.allowed-domains` is not configured, only GitHub-owned domains (`github.com`, `github.io`, `githubusercontent.com`, `githubassets.com`, `githubapp.com`, `github.dev`) are allowed
- **Custom restrictions**: If `output.allowed-domains` is configured, only HTTPS URLs matching those domain patterns are preserved
...
### With default behavior (no `output.allowed-domains` configured)
```
...
### With `output.allowed-domains: [github.com]`
```
Visit https://github.com for code and [filtered] for bad stuff.
Also check [API docs](https://api.github.com/docs) and bad link [filtered].
```

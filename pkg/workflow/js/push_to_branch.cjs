async function main() {
  ...
  if (!patchContent || !patchContent.trim() || patchContent.includes('Failed to generate patch')) {
    core.setFailed('Patch file is empty or contains error message - cannot push without changes');
    return;
  }
  ...
  try {
    ...
    console.error('Failed to apply patch:', error instanceof Error ? error.message : String(error));

...
describe('add_labels.cjs', () => {
  ...
  beforeEach(() => {
    ...
    const scriptPath = path.join(process.cwd(), 'pkg/workflow/js/add_labels.cjs');
  ...
  describe('Environment variable validation', () => {
    ...
    it('should fail when max count is invalid', async () => {
      ...
      expect(mockCore.setFailed).toHaveBeenCalledWith('Invalid max-count value: invalid. Must be a positive integer');
      expect(mockGithub.rest.issues.addLabels).not.toHaveBeenCalled();
    });
    ...
    it('should fail when max count is zero', async () => {
      ...
      expect(mockCore.setFailed).toHaveBeenCalledWith('Invalid max-count value: 0. Must be a positive integer');
      expect(mockGithub.rest.issues.addLabels).not.toHaveBeenCalled();
    });
  ...
  describe('GitHub API integration', () => {
    it('should successfully add labels to issue', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Successfully added 2 labels to issue #123');
      expect(mockCore.setOutput).toHaveBeenCalledWith('labels_added', 'bug\nenhancement');
    ...
    it('should successfully add labels to pull request', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Successfully added 1 labels to pull request #456');
    ...
    it('should handle GitHub API errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to add labels:', 'Label does not exist');
    ...
    it('should handle non-Error objects in catch block', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to add labels:', 'Something went wrong');
  ...
  describe('Output and logging', () => {
    ...
    it('should log allowed labels and max count', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Allowed labels:', ['bug', 'enhancement', 'feature']);
    ...
    it('should log requested labels', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Requested labels:', ['bug', 'enhancement', 'invalid']);
    ...
    it('should log final labels being added', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Adding 2 labels to issue #123:', ['bug', 'enhancement']);

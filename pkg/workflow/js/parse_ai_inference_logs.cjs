function main() {
  ...
  if (!logFilePath) {
    console.error(
      "Usage: node parse_ai_inference_logs.cjs <log_file_path> [--verbose]"
    );
    process.exit(1);
  }
      ...
      try {
        ...
        if (jsonData.usage) {
          if (jsonData.usage.total_tokens) {
            ...
            if (verbose) {
              console.log(
                `Found usage.total_tokens: ${jsonData.usage.total_tokens}`
              );
            }
          } else {
            // Sum input and output tokens if total not available
            const inputTokens =
              jsonData.usage.prompt_tokens || jsonData.usage.input_tokens || 0;
            const outputTokens =
              jsonData.usage.completion_tokens ||
              jsonData.usage.output_tokens ||
              0;
            if (inputTokens || outputTokens) {
              ...
              if (verbose) {
                console.log(
                  `Found input_tokens: ${inputTokens}, output_tokens: ${outputTokens}, total: ${tokens}`
                );
              }
            }
          }
        ...
        if (verbose) {
          console.log(
            `Failed to parse JSON line: ${trimmed.substring(0, 100)}...`
          );
        }
      }
    }

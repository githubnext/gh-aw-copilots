...
describe('setup_agent_output.cjs', () => {
  ...
  beforeEach(() => {
    ...
    const scriptPath = path.join(process.cwd(), 'pkg/workflow/js/setup_agent_output.cjs');
  ...
  describe('main function', () => {
    it('should create output file and set environment variables', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Created agentic output file:', outputFile);
    ...
    it('should verify file existence and throw error if file creation fails', async () => {
      ...
      try {
        ...
        expect(error.message).toMatch(/^Failed to create output file: \/tmp\/aw_output_[0-9a-f]{16}\.txt$/);
      }

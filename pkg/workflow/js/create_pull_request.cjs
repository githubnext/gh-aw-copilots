...
async function main() {
  ...
  // Prepare the body content and title, optionally escape HTML
  let body = bodyLines.join('\n').trim();
  let processedTitle = title;
  
  if (process.env.GITHUB_AW_PR_ALLOW_HTML !== 'true') {
    // Escape HTML characters and neutralize mentions
    const escapeHtml = (/** @type {string} */ s) => s.replace(/[&<>"]/g, (/** @type {string} */ c) =>
      c === "&" ? "&amp;" :
      c === "<" ? "&lt;" :
      c === ">" ? "&gt;" :
      c === '"' ? "&quot;" : c);
    const neutralizeMentions = (/** @type {string} */ s) => s.replace(/(^|[^\w`])@([A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?(?:\/[A-Za-z0-9._-]+)?)/g,
                                                (/** @type {string} */ _m, /** @type {string} */ p1, /** @type {string} */ p2) => `${p1}\`@${p2}\``);
    
    body = neutralizeMentions(escapeHtml(body));
    processedTitle = neutralizeMentions(escapeHtml(title));
    console.log('HTML content escaped and mentions neutralized in pull request title and body');
  }
  ...
  console.log('Creating pull request with title:', processedTitle);
  ...
  execSync(`git commit -m "Add agent output: ${processedTitle}"`, { stdio: 'inherit' });

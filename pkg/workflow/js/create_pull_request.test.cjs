...
const createTestableFunction = (scriptContent) => {
  ...
  const mainFunctionMatch = scriptContent.match(/async function main\(\) \{([\s\S]*?)\}\s*await main\(\);?$/);
...
describe('create_pull_request.cjs', () => {
  ...
  it('should throw error when patch file is empty', async () => {
    ...
    await expect(mainFunction()).rejects.toThrow('Patch file is empty or contains error message - cannot create pull request without changes');
  });
  ...
  it('should create pull request successfully with valid input', async () => {
    ...
    expect(mockDependencies.execSync).toHaveBeenCalledWith('git config --global user.email "action@github.com"', { stdio: 'inherit' });
    expect(mockDependencies.execSync).toHaveBeenCalledWith('git config --global user.name "GitHub Action"', { stdio: 'inherit' });
    expect(mockDependencies.execSync).toHaveBeenCalledWith('git checkout -b test-workflow/1234567890abcdef', { stdio: 'inherit' });
    ...
    expect(mockDependencies.execSync).toHaveBeenCalledWith('git commit -m "Add agent output: New Feature"', { stdio: 'inherit' });
    expect(mockDependencies.execSync).toHaveBeenCalledWith('git push origin test-workflow/1234567890abcdef', { stdio: 'inherit' });

async function main() {
  ...
  console.log(`Can update status: ${canUpdateStatus}, title: ${canUpdateTitle}, body: ${canUpdateBody}`);
  ...
  for (let i = 0; i < updateItems.length; i++) {
    ...
    if (updateTarget === "*") {
      ...
      if (updateItem.issue_number) {
        ...
        if (isNaN(issueNumber) || issueNumber <= 0) {
          console.log(`Invalid issue number specified: ${updateItem.issue_number}`);
          continue;
        }
      } else {
      ...
      if (isNaN(issueNumber) || issueNumber <= 0) {
        console.log(`Invalid issue number in target configuration: ${updateTarget}`);
        continue;
      }
    } else {
    ...
    if (canUpdateStatus && updateItem.status !== undefined) {
      ...
      if (updateItem.status === 'open' || updateItem.status === 'closed') {
        ...
        console.log(`Invalid status value: ${updateItem.status}. Must be 'open' or 'closed'`);
      }
    }
